...
<html>
	<head>
		...
		<ul>
			<li><a style="color: white;" class="active" href="lists_got.php">Odebrane</a></li>
	...
	<body>
		<h2>Odebrane:</h2>
		<input type="text" id="myInput" onkeyup="myFunction()" placeholder="Wyszukaj po adresatach" title="Type in a name">
		<table id="myTable">
			<tr class="header">
			...
			<?php
				...
				$query = "SELECT * FROM `letters` WHERE do='$login_session' ORDER BY `letters`.`id` DESC";
				...
				while ($row = mysqli_fetch_array($results))
				{
					...
					$podglad = $row['tresc'];
					$podglad = substr($podglad, 0, 50);
					echo "
					<tr>
						<td> <a href=".$gdzie . " target='_blank'> " . $row['od'] ." </a></td> 
						<td> <a href=".$gdzie . " target='_blank'> " . $row['datawyslania'] ." </a></td> 
						<td> <a href=".$gdzie . " target='_blank' style='font-size: 15px;'> " . $podglad ."... </a></td> 
					</tr>
					";
				}
